    ...
    TreeTable.prototype.init = function () {
        ...
        if (options.height) {  // 固定表头
            ...
            if (options.height.indexOf('full-') === 0) {  // 差值高度
                var h = parseFloat(options.height.substring(5)) + (options.toolbar?components.$toolbar.outerHeight():0)
        ...
        function each(data, pi) {
            for (var i = 0; i < data.length; i++) {
                if (data[i][options.tree.idName] == id)
                    return pi !== undefined ? pi + '-' + i : i;
                if (data[i][options.tree.childName])
                    var indexId = each(data[i][options.tree.childName], pi !== undefined ? pi + '-' + i : i);
