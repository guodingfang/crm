...
layui.define('form', function(exports){
  ...
  Class.prototype.setCheckbox = function(elem, item, elemCheckbox){
    ...
    //同步子节点选中状态
    if(options.linkChild && (typeof item.children === 'object' || elem.find('.'+ELEM_PACK)[0])){
  ...
  Class.prototype.setChecked = function(checkedId){
    ...
    that.elem.find('.'+ELEM_SET).each(function(i, item){
      ...
      //若返回数字
      if(typeof checkedId === 'number' || typeof checkedId === 'string'){
        if(thisId == checkedId){
          if(!input[0].checked){
        	  if(options.linkChild && options.linkParent)
        		  reInput.click();
        	  else{
        	      ...
        	      options.oncheck && options.oncheck({
        	        ...
        	        ,data: function(){
        	        	var value;
        	        	layui.each(options.data,function(i,d){
        	        		if(d.id=checkedId)value=d;return true;
        	        		if(d.children)
        	        			layui.each(d.children,function(j,c){
                	        		if(d.id=checkedId)value=d;return true;
        	        			});
        	        	});
        	        	return value;
