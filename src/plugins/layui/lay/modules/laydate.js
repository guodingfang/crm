...
;!function(window){
  ...
  */
    ...
    lay.each(new Array(2), function(i){
      ...
      lay.each(new Array(6), function(i){ //表体
        ...
        lay.each(new Array(7), function(j){
          if(i === 0){
            var th = lay.elem('th');
            th.innerHTML = lang.weeks[(j+options.weekStart)%7];//计算每周开始日
  ...
  //系统日期
  Class.prototype.systemDate = laydate.parseDate = function(newDate){
  ...
  Class.prototype.calendar = function(value, index, type){
    ...
    startWeek = (thisDate.getDay()+(7-options.weekStart))%7;//修改为根据配置项定位开始星期数
    ...
    lay.each(tds, function(index_, item){
      ...
      if(options.markToday && laydate.format(new Date(),'yyyy-M-d')===item.attr('lay-ymd'))
    	  item.html('<span class="laydate-day-mark">今</span>');
    });  
  ...
  //格式化日期
  laydate.format=function(dateTime,format){
	  var format_array = format.match(new RegExp('yyyy|y|MM|M|dd|d|HH|H|mm|m|ss|s|.', 'g')) || [];
	  if(dateTime instanceof Date){
		  dateTime=laydate.parseDate(dateTime);
		  dateTime.month=dateTime.month+1;
	  }
	  lay.each(format_array, function(i, item){
	      if(/yyyy|y/.test(item)){ //年
	    	  format_array[i] = lay.digit(dateTime.year, item.length);
	      } else if(/MM|M/.test(item)){ //月
	    	  format_array[i] = lay.digit(dateTime.month, item.length);
	      } else if(/dd|d/.test(item)){ //日
	    	  format_array[i] = lay.digit(dateTime.date, item.length);
	      } else if(/HH|H/.test(item)){ //时
	    	  format_array[i] = lay.digit(dateTime.hours, item.length);
	      } else if(/mm|m/.test(item)){ //分
	    	  format_array[i] = lay.digit(dateTime.minutes, item.length);
	      } else if(/ss|s/.test(item)){ //秒
	    	  format_array[i] = lay.digit(dateTime.seconds, item.length);
	      }
	    });
