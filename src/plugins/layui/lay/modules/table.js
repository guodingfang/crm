...
layui.define(['laytpl', 'laypage', 'layer', 'form', 'util'], function(exports){
  ...
  ,table = {
    config: {
      ...
      ,isDisabled: 'LAY_DISABLED' //是否选中状态的字段名
  ...
  ,jiaobiaoData = function(item3, content, tplData, text){ //表头数据、原始内容、表体数据、是否只返回文本
      var jiaobiaoValue = item3.jiaobiao ? function(){
          return typeof item3.jiaobiao === 'function'
              ? item3.jiaobiao(tplData)
              : laytpl($(item3.jiaobiao).html() || String(content)).render(tplData)
      }() : content;
    return jiaobiaoValue;
  }
  ,isDisabledData = function(item3, content, tplData, text){ //表头数据、原始内容、表体数据、是否只返回文本
      var isDisabledValue = item3.isDisabled ? function(){
          return typeof item3.isDisabled === 'function'
              ? item3.isDisabled(tplData)
              : laytpl($(item3.isDisabled).html() || String(content)).render(tplData)
      }() : content;
      return isDisabledValue;
  }
  ,isCanEdit = function(item3, content, tplData, text){ //表头数据、原始内容、表体数据、是否只返回文本
      var jiaobiaoValue = item3.edit ? function(){
          return typeof item3.edit === 'function' ? item3.edit(tplData) : item3.edit
      }() : content;
      return jiaobiaoValue;
  }
  ,isCanStyle = function(item3, content, tplData, text){ //表头数据、原始内容、表体数据、是否只返回文本
      var jiaobiaoValue = item3.style ? function(){
        return typeof item3.style === 'function' ? item3.style(tplData) : item3.style
      }() : content;
      return jiaobiaoValue;
  }
    ...
    var getAutoWidth = function(back){
      ...
      layui.each(options.cols, function(i1, item1){
        layui.each(item1, function(i2, item2){
          ...
          if(!back){
            width = item2.width==='auto'?60:Number(item2.width) || 0;
  ...
  Class.prototype.renderData = function(res, curr, count, sort){
    ...
    ,render = function(){ //后续性能提升的重点
      ...
      layui.each(data, function(i1, item1){
        ...
        that.eachCols(function(i3, item3){
          ...
          var td = ['<td data-field="'+ field +'" data-key="'+ key +'" '+ function(){ //追加各种属性
            var attr = [];
            if(item3.edit) {
              var isEdit = isCanEdit(item3, content, tplData);
              if (isEdit) {
            ...
            if(item3.style) {
              var isStyle = isCanStyle(item3, content, tplData);
              if (isStyle && typeof isStyle == 'string') {
                attr.push('style="'+ isStyle +'"'); //自定义样式
              }
            }
            ...
            ,'<div class="layui-table-cell laytable-cell-'+ function(){ //返回对应的CSS类标识
              ...
              var isDisabled = table.config.isDisabled;

              //渲染不同风格的列
              switch(item3.type){
                case 'checkbox':
                  if (item3.isDisabled){
                      var isDisabledValue = isDisabledData(item3, content, tplData);
                      if (typeof isDisabledValue === 'boolean') {
                          item1[isDisabled] = isDisabledValue;
                          if (isDisabledValue) {
                              return '<input type="checkbox" name="layTableCheckbox" lay-skin="primary" disabled>';
                          } else {
          ...
          if (item3.jiaobiao) {
            var jiaobiaoValue = jiaobiaoData(item3, content, tplData);
            if (typeof jiaobiaoValue === 'boolean') {
              if (jiaobiaoValue) {
                td.push('<div class="layui-cell-jiaobiao" lay-event="jiaobiao"></div>');
              }
            } else {
              td.push('<div class="layui-cell-jiaobiao" lay-event="jiaobiao" style="background: linear-gradient(45deg,rgba(0,0,0,0) 50%, '+jiaobiaoValue+' 50%);"></div>');
            }
          }
          td.push('</td>');
          td = td.join('');
      ...
      layer.close(that.tipsIndex,window);
  ...
  //同步选中值状态
  Class.prototype.setCheckData = function(index, checked, isDis){
    var that = this
    ,options = that.config
    ...
    if (isDis) {
      thisData[index][options.isDisabled] = isDis;
    }
  };
  ...
  Class.prototype.events = function(){
    ...
    that.elem.on('click', 'input[name="layTableCheckbox"]+', function(){ //替代元素的 click 事件
      ...
      if(isAll){
        childs.each(function(i, item){
            if (col.isDisabled){
                var rowData,
                    dataLength = table.cache[that.key].length;
                if (dataLength <= i) {
                    rowData = table.cache[that.key][i-dataLength];
                } else {
                    rowData = table.cache[that.key][i];
                }
                var isDisabledValue = isDisabledData(col, "", rowData);
                if (typeof isDisabledValue === 'boolean') {
                    if (isDisabledValue) {
                        item.checked = false;
                        that.setCheckData(i, false, isDisabledValue);
                    } else {
                        item.checked = checked;
                        that.setCheckData(i, checked, isDisabledValue);
                    }
                }
            } else {
                item.checked = checked;
        ...
        that.renderForm('checkbox');
          ...
          if (col.isDisabled){
              var isDisabledValue = isDisabledData(col, "", table.cache[that.key][index]);
              if (typeof isDisabledValue === 'boolean') {
                  if (isDisabledValue) {
                      that.setCheckData(index, false, isDisabledValue);
                  } else {
                      that.setCheckData(index, checked, isDisabledValue);
                  }
              }
          } else {
    ...
    that.layBody.on('change', '.'+ELEM_EDIT, function(){
      ...
      if(editType!=='autocomplete'){
    	  var oldValue=$.extend({}, data, {})[field];
    ...
    that.layBody.on('click', 'td', function(e){
      ...
      //显示编辑表单
      if(editType){
        var input = editType=='textarea'?$('<textarea class="layui-textarea '+ ELEM_EDIT +'"></textarea>'):$('<input class="layui-input '+ ELEM_EDIT +'">');
        ...
        if(editType=='autocomplete'){
            ...
            var c={};
        	that.eachCols(function(i, item){
                if(item.field == field && item.autocomplete){
                	c = item.autocomplete;
                }
            });
        	c.oncheck=function(item){
        	      layui.event.call(othis, MOD_NAME, 'edit('+ filter +')', commonMember.call(othis, {
        	        value: item
        	        ,field: field
        	      }));
        	}
        	c.index=othis.parents('tr').eq(0).data('index');
        	c.onhide=function(){othis.data('off', false);input.remove();}
        	input.autocomplete(c);
        }
        input.focus();
        layui.stope(e);
        return;
      }
    }).on('mouseenter', 'td '+ELEM_CELL, function(){
    	if(options['gridtip'] != undefined && options.gridtip == "hover"){//鼠标移入单元格悬浮展开
			var elemCell = $(this)
		      ,td = $(this).parent();
    		if(elemCell.prop('scrollWidth') > elemCell.outerWidth()){
    		      that.tipsIndex = layer.tips([
    		        '<div class="layui-table-tips-main layui-table-tips-hover" style="margin-top: -'+ (elemCell.height() + 16) +'px;width:'+(td.width()-30)+'px;max-width:'+td.width()+'px;'+ function(){
    		          if(options.size === 'sm'){
    		        ...
    		        ,success: function(layero, index){
    		          layero.find(".layui-layer-content").on("mouseleave",function(){//鼠标移出隐藏tip
    		        	  layer.close(index,window);
    		          }).on("click",function(){//如果是可编辑的触发点击事件
    		        	  td.click();
    		        	  layer.close(index,window);
    		          });
    		        }
    	...
    	if(options['gridtip'] == undefined || options.gridtip=="tipdown")
    ...
    that.layBody.on('click', '.'+ ELEM_GRID_DOWN, function(e){
      ...
      that.tipsIndex = layer.tips([
        '<div class="layui-table-tips-main" style="margin-top: -'+ (elemCell.height() + 16) +'px;max-width:'+td.width()*1.5+'px;'+ function(){
          if(options.size === 'sm'){
            return 'padding: 4px 15px; font-size: 12px;';
          }
          if(options.size === 'lg'){
    ...
    that.layBody.find("select").attr("lay-filter",'LAY-table-select-'+ that.index);
    //绑定行内的select组件,转发为edit事件
    form.on("select("+'LAY-table-select-'+ that.index+")",function(obj){
    	var td=obj.othis.parents("td");
    	layui.event.call(this, MOD_NAME, 'edit('+ filter +')', commonMember.call(obj.othis, {
    		value:obj.value,
    		field:td.data('field')
    	}));
    });
  ...
  table.checkStatus = function(id){
    ...
    layui.each(data, function(i, item){
      ...
      if(item[table.config.isDisabled]){
        dataLength--;
      }
    ...
    return {
      data: arr //选中的数据
      ,isAll: dataLength ? (nums === (dataLength - invalidNum)) : false //是否全选
    };
  };
